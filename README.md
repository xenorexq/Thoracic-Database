...
* AJCC 分期参考：在患者/治疗页右侧提供肺癌和食管癌 TNM 定义及分期参考，供录入时查阅。用户可通过点击“Lung”或“Eso”按钮手动选择不同癌种的参考内容，文本框宽度可通过拖动分隔条调整。
...
5. **AJCC 分期参考**：右侧提供肺癌与食管癌 TNM 定义参考表。点击“Lung”或“Eso”按钮即可查看对应内容；此内容仅供参考，不自动根据 TNM 选择更新。
6. **数据校验**：
...
* **移除 TNM 分期映射功能**：取消了自动根据 TNM 计算临床分期的功能，包括去除患者/治疗页的“计算”按钮、临床分期显示以及导出数据中的 `lung_stage` 和 `eso_stage` 字段。数据库结构保持不变，但分期映射查询不再执行。
* **调整 AJCC 分期参考**：右侧的 AJCC 分期参考区域加入了可调节宽度的分隔条，用户可以拖动调整参考栏和表单之间的宽度，改善阅读体验。分期参考仅供查阅，不再自动根据癌种切换。
* **版本信息更新**：关于对话框和 README 标题更新为 v1.30。
