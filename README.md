...
- **模块解耦**
  - 新增 `db/importer.py` 模块，将数据库合并逻辑抽离为独立函数，便于维护和单元测试。
- **性能优化**
  - 合并操作改为逐行读取和插入，使用参数化查询避免 SQL 拼接；对导入过程增加日志记录。
- **后台导入**
  - 导入操作在后台线程中执行，不再阻塞 UI，并将进度反馈到状态栏。
- **输入校验**
  - 新增对住院号格式（仅字母和数字）和日期格式（出生年月、治疗日期）的验证，防止无效数据进入数据库。
- **错误日志**
  - 新增 `importer.log` 文件记录导入过程中的错误，方便诊断。
- **版本号更新**
  - 更新 `VERSION.txt` 为 `v3.2`，并同步修改“关于”对话框和 README。
