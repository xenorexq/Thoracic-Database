...
- **稳定性改进 - Bug修复版本**：
  - 修复了10个潜在bug，包括除零错误、类型转换失败、资源泄漏等
  - **异常处理增强**：改进导入功能的错误处理，不再静默失败
  - **类型转换安全**：所有数字字段添加安全转换，提供友好的错误提示
  - **线程安全改进**：多线程导出使用独立数据库连接，避免锁定
  - **资源管理优化**：确保所有数据库连接正确关闭，防止资源泄漏
  - **测试覆盖**：新增完整的功能测试套件 `test_all_functions.py`
...
- **Bug修复**：
  ...
  - 修复sqlite3.Row兼容性问题
  - 添加详细的字段验证和错误提示
  - 使用safe_str()过滤"None"字符串

### v3.6.0 (2025-11-25)

- **性能优化 - 多线程导出**：
  - 新增并行处理模块 (`export/parallel.py`)，支持多线程并发导出
  - **Excel 导出加速**：数据获取阶段使用线程池并行读取多个表，性能提升 2-4 倍
  - **CSV 导出加速**：数据获取和文件写入双重并行化，性能提升 3-5 倍
  - **实时进度显示**：导出过程显示实时进度条和百分比
  - **后台处理**：导出在后台线程执行，UI 界面不卡顿，用户可继续使用其他功能
  - **智能线程管理**：根据表数量和 CPU 核心数自动调整线程池大小（默认 4 线程）
...
- **技术改进**：
  ...
  - 添加进度跟踪器 (`ExportProgress`) 统一管理进度回调
  - 导出函数支持可选的 `progress_callback` 参数
  - 向后兼容：旧版本调用方式仍然支持

- **性能基准**（实测数据）：
  - 100 患者：2.5秒 → 1.2秒（提升 2.1x）
  - 1000 患者：28秒 → 8秒（提升 3.5x）
  - 5000 患者：145秒 → 38秒（提升 3.8x）

- **文档更新**：
  - 新增 `EXPORT_OPTIMIZATION.md` 详细说明优化方案
  - 新增 `test_export_performance.py` 性能测试脚本
