...
class PatientTab(ttk.Frame):
    ...
    def _build_widgets(self) -> None:
        ...
        # 垂直滚动条放在 canvas 左侧，方便右侧留出位置显示分期参考
        ...
        # 在 TNM 选择后可点击此按钮计算临床分期
        ttk.Button(self.lung_frame, text="计算", command=self.calculate_lung_stage).grid(row=lung_row, column=6, sticky="e", padx=5)
        ttk.Label(self.lung_frame, text="临床分期:").grid(row=lung_row, column=7, sticky="e", padx=5)
        self.lung_stage_label = ttk.Label(self.lung_frame, text="", foreground="blue", font=("Arial", 10, "bold"))
        self.lung_stage_label.grid(row=lung_row, column=8, sticky="w", padx=5)
        ...
        # 在食管癌 TNM 等选择后可点击此按钮计算临床分期
        ttk.Button(self.eso_frame, text="计算", command=self.calculate_eso_stage).grid(row=eso_row, column=2, sticky="e", padx=5)
        ttk.Label(self.eso_frame, text="临床分期:").grid(row=eso_row, column=3, sticky="e", padx=5)
        self.eso_stage_label = ttk.Label(self.eso_frame, text="", foreground="blue", font=("Arial", 10, "bold"))
        # 临床分期文本占 3 列以保证足够宽度
        self.eso_stage_label.grid(row=eso_row, column=4, columnspan=3, sticky="w", padx=5)
        ...
        # === 分期参考区域（右侧） ===
        # 创建一个文本框放在整个患者/治疗页的右侧，用于显示肺癌或食管癌的 AJCC TNM 定义。
        # 不可编辑，通过点击按钮手动显示不同癌种的参考信息。
        self.stage_frame = ttk.LabelFrame(self, text="AJCC 分期参考")
        self.stage_frame.pack(side="right", fill="both", expand=False, padx=5, pady=5)
        # 按钮区域：用户可根据需要点击“Lung”或“Eso”按钮显示对应的参考表
        ...
        # 使用自定义 HTMLScrolledText 显示 Markdown 内容。此控件支持
        # 基本的 HTML 渲染功能，包括标题、粗体、水平线和简单表格。
        self.stage_text = HTMLScrolledText(self.stage_frame, width=40, wrap="word")
    ...
    def _update_stage(self):
        ...
        if cancer_type == "肺癌":
            ...
            if t and n and m:
                # 调用 get_lung_stage 时传入 Database 实例而非连接对象
                stage = get_lung_stage(self.db, t, n, m)  # type: ignore[arg-type]
            ...
            if t and n and m and hist and grade and loc:
                # 调用 get_eso_stage 时传入 Database 实例
                stage = get_eso_stage(self.db, t, n, m, hist, grade, loc)  # type: ignore[arg-type]
                self.eso_stage_label.config(text=stage or "未匹配")
            else:
                self.eso_stage_label.config(text="")

        # 不再自动更新右侧分期参考信息。用户可通过点击右侧按钮手动查看参考表。
...
<hr />"""
    ...
    def calculate_lung_stage(self) -> None:
        """手动计算并更新肺癌临床分期显示。"""
        # 使用统一计算函数，忽略癌种自动映射
        self.compute_stage("肺癌")

    def calculate_eso_stage(self) -> None:
        """手动计算并更新食管癌临床分期显示。"""
        self.compute_stage("食管癌")

    # ======== 新增：临床分期计算辅助方法 ========
    def compute_stage(self, cancer_type: str) -> None:
        """
        根据当前输入的 TNM 信息，手动计算临床分期。通过点击
        肺癌或食管癌区域的“计算”按钮触发调用。

        :param cancer_type: "肺癌" 或 "食管癌"
        """
        try:
            if cancer_type == "肺癌":
                # 读取并规范化 cTNM
                t = self.lung_t_var.get() or ""
                n = self.lung_n_var.get() or ""
                m = self.lung_m_var.get() or ""
                t, n, m = self._normalize_tnm(t, n, m)
                # 必须同时存在 T/N/M
                if not (t and n and m):
                    self.lung_stage_label.config(text="(TNM不完整)")
                    return
                # 调用分期映射函数
                # 传递 Database 实例而不是连接对象
                stage = get_lung_stage(self.db, t, n, m)  # type: ignore[arg-type]
                self.lung_stage_label.config(text=stage or "未匹配")
            elif cancer_type == "食管癌":
                # 读取并规范化 cTNM
                t = self.eso_t_var.get() or ""
                n = self.eso_n_var.get() or ""
                m = self.eso_m_var.get() or ""
                t, n, m = self._normalize_tnm(t, n, m)
                # 食管癌还需要组织学、分级和部位
                hist = self.eso_hist_var.get() or ""
                grade = self.eso_grade_var.get() or ""
                loc = self.eso_loc_var.get() or ""
                if not (t and n and m and hist and grade and loc):
                    self.eso_stage_label.config(text="(TNM不完整)")
                    return
                # 调用映射函数
                stage = get_eso_stage(self.db, t, n, m, hist, grade, loc)  # type: ignore[arg-type]
                self.eso_stage_label.config(text=stage or "未匹配")
            ...
            # 出现异常时弹窗提示错误信息
            messagebox.showerror("计算分期失败", str(e))

    def _normalize_tnm(self, t: str, n: str, m: str) -> tuple[str, str, str]:
        """
        将 TNM 值统一为映射表的存储格式。

        映射表中的 ``t``/``n``/``m`` 字段通常使用小写字母作为后缀（例如
        ``1a``, ``2b``, ``1c1``），不包含前缀 ``T``/``N``/``M``。因此此函数
        去除任何前缀并将字母后缀转换为小写，同时去除空格。若输入为空则
        返回空字符串。
        """
        ...
        def norm(value: str) -> str:
            ...
            if not v:
                return ""
            # 移除前缀 T/N/M（不区分大小写）
            if v[0] in {"T", "N", "M", "t", "n", "m"}:
                v = v[1:]
            # 转小写，保证字母后缀统一
            return v.lower()

        return norm(t), norm(n), norm(m)
